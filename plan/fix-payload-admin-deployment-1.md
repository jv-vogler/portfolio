...
| TASK-026 | **Make server actions resilient to missing Payload env vars**: Create `src/lib/payload.ts` with a `getPayloadSafe()` helper that catches `payloadInitError` and returns `null`. Update all server actions (`blog.ts`, `portfolio.ts`, `skills.ts`) to use this helper and return empty arrays when Payload can't initialise. This ensures `generateStaticParams` and other build-time calls don't crash when `PAYLOAD_SECRET` / `POSTGRES_URL` are absent (e.g. CI). Pages are generated on-demand via ISR at runtime instead. | ✅                  | 2026-03-02                                                                                                                                                                                                                                 |
...
| Task     | Description                                                                                                                                                                                                                                             | Completed                                             | Date       |
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------- | ---------- |
| TASK-020 | **Local verification**: Run `pnpm dev`, navigate to `http://localhost:3000/admin`. Confirm the login page renders. Create an admin user, log in, verify collections (Users, Media, Posts, Projects, Skills) are visible and operable.                   | ⚠️ Partial (smoke checks complete; auth CRUD pending) | 2026-03-02 |
| TASK-021 | **Build verification**: Run `pnpm build` locally. Confirm no TypeScript errors and the build succeeds.                                                                                                                                                  | ✅                                                    | 2026-03-02 |
...
- **FILE-008**: `src/lib/payload.ts` — New file: safe `getPayload` wrapper that returns `null` when Payload can't initialise
- **FILE-009**: `src/app/actions/blog.ts` — Use `getPayloadSafe` instead of direct `getPayload`; return `[]` when unavailable
- **FILE-010**: `src/app/actions/portfolio.ts` — Use `getPayloadSafe` instead of direct `getPayload`; return `[]`/`null` when unavailable
- **FILE-011**: `src/app/actions/skills.ts` — Use `getPayloadSafe` instead of direct `getPayload`; return `[]` when unavailable
