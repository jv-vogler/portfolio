...
| Task      | Description                                                                                                                                                                                                                                           | Completed | Date       |
| --------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------- | ---------- |
| TASK-1301 | Run Oxlint across the entire `src/` directory — fix all reported issues. Configure any project-specific rule overrides in `oxlint.json`.                                                                                                              | ✅        | 2026-02-28 |
| TASK-1302 | Run Oxfmt across the entire `src/` directory — `pnpm format` to auto-format all files. Verify with `pnpm format:check`.                                                                                                                               | ✅        | 2026-02-28 |
| TASK-1303 | Run `pnpm typecheck` (tsc) and `pnpm typecheck:go` (tsgo) — fix all type errors. Document any tsgo-specific issues or incompatibilities encountered.                                                                                                  | ✅        | 2026-02-28 |
| TASK-1304 | Run `pnpm build` — verify the production build succeeds with zero errors. Check for any build warnings and address them.                                                                                                                              | ✅        | 2026-02-28 |
| TASK-1305 | Accessibility audit — manually test with keyboard navigation. Ensure all interactive elements are focusable, images have alt text, color contrast meets WCAG AA. Use `aria-label` on icon-only buttons (theme toggle, locale switcher, social links). | ✅        | 2026-02-28 |
...
| TASK-1307 | Verify the dependency rule — run `grep -rn "from '@/ui/\|from '@/app/" src/core/ src/lib/` — must return zero results. Run `grep -rn "from '@/app/" src/lib/` — must return zero results.                                                             | ✅        | 2026-02-28 |
| TASK-1308 | Update `README.md` — Document the new tech stack, project structure, development setup (pnpm install, env vars, dev server), deployment process, blog authoring workflow, and architecture diagram.                                                   | ✅        | 2026-02-28 |
| TASK-1309 | _(Optional, future)_ Set up CI/CD pipeline — GitHub Actions workflow that runs: `pnpm install`, `pnpm typecheck`, `pnpm lint`, `pnpm format:check`, `pnpm build` on every PR. Vercel handles deployment automatically.                                | ✅        | 2026-02-28 |
