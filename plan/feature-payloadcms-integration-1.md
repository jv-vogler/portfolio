...
| Task     | Description                                                                                                                                                                                                                                                                                                                                                         | Completed                                                          | Date       |
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------ | ---------- |
| TASK-011 | Create `src/collections/Users.ts`: `CollectionConfig` with `slug: 'users'`, `auth: true`, `admin: { useAsTitle: 'email' }`, fields: `[{ name: 'name', type: 'text' }]`. Access: admin-only for create/update/delete; read allowed for authenticated users.                                                                                                          | ✅ (added `role` select field; access guards use `req.user?.role`) | 2026-03-01 |
| TASK-012 | Create `src/collections/Media.ts`: `CollectionConfig` with `slug: 'media'`, `upload: { staticDir: path.resolve(dirname, '../../public/media'), mimeTypes: ['image/*'], imageSizes: [{ name: 'thumbnail', width: 400 }, { name: 'card', width: 768 }, { name: 'hero', width: 1400 }] }`, fields: `[{ name: 'alt', type: 'text', required: true, localized: true }]`. | ✅ (local disk storage; Vercel Blob deferred to Phase 10)          | 2026-03-01 |
| TASK-013 | Register `Users` and `Media` collections in `src/payload.config.ts` under `collections: [Users, Media]`.                                                                                                                                                                                                                                                            | ✅ (also added `admin.user: 'users'`)                              | 2026-03-01 |
| TASK-014 | Run `pnpm dev`, verify admin panel shows Users and Media collections, create a test admin user at `/admin`.                                                                                                                                                                                                                                                         | ✅                                                                 | 2026-03-01 |
...
| Task     | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Completed                                                                                                 | Date       |
| -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- | ---------- |
| TASK-015 | Create `src/collections/Posts.ts`: `CollectionConfig` with `slug: 'posts'`, `admin: { useAsTitle: 'title', defaultColumns: ['title', 'status', 'publishedDate', 'updatedAt'] }`, `versions: { drafts: true }`, `fields`: `title` (text, required, localized), `slug` (text, unique, index, admin.position: 'sidebar'), `description` (textarea, required, localized), `content` (richText/lexical, required, localized), `tags` (array of text fields, or alternatively a relationship to a Tags collection), `publishedDate` (date, required, admin.position: 'sidebar'), `status` (select: 'draft'/'published', defaultValue: 'draft'). | ✅ (`_status` auto-managed by `versions.drafts`)                                                          | 2026-03-01 |
| TASK-016 | Add `beforeChange` hook to `Posts` collection that auto-generates `slug` from `title` on create if slug is empty (using a slugify utility).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | ✅                                                                                                        | 2026-03-01 |
| TASK-017 | Add `afterChange` hook to `Posts` collection that calls `revalidatePath('/[locale]/blog')` and `revalidatePath('/[locale]/blog/[slug]')` for both locales when a post is created/updated/deleted, using `context` flag to prevent loops.                                                                                                                                                                                                                                                                                                                                                                                                  | ✅ (`afterDelete` hook also added)                                                                        | 2026-03-01 |
| TASK-018 | Register `Posts` collection in `src/payload.config.ts`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | ✅                                                                                                        | 2026-03-01 |
| TASK-019 | Update `src/core/blog.ts`: Keep the `Blog` namespace and types but update `Blog.Post` to align with Payload's generated type. Add a mapping function `fromPayload(doc: PayloadPost): Blog.Post` that maps Payload document shape to the existing `Blog.Post` interface. Keep `sortByDate` and `filterPublished` utility functions. The `Blog.Post` type should remain: `{ slug, title, description, date, tags, published, locale }`.                                                                                                                                                                                                     | ✅ (`PayloadPost` interface exported; `Frontmatter` type removed)                                         | 2026-03-01 |
| TASK-020 | Rewrite `src/app/actions/blog.ts`: Replace `getPostSlugs`/`getPostBySlug` imports with Payload Local API calls. `getAllPosts(locale)` → `const payload = await getPayload({ config }); const { docs } = await payload.find({ collection: 'posts', locale, where: { status: { equals: 'published' } }, sort: '-publishedDate', limit: 100 }); return docs.map(Blog.fromPayload)`. `getPost(slug, locale)` → `const { docs } = await payload.find({ collection: 'posts', locale, where: { slug: { equals: slug } }, limit: 1 }); return { post: Blog.fromPayload(docs[0]), content: docs[0].content }`.                                     | ✅ (uses `_status` filter; `overrideAccess: true`)                                                        | 2026-03-01 |
| TASK-021 | Update `src/ui/blog/components/BlogPost.tsx`: Replace `MDXRemote` rendering with Payload's Lexical rich text serializer. Install and use `@payloadcms/richtext-lexical/react` `RichText` component to render the serialized Lexical content. The component signature changes from `{ content: string }` to `{ content: SerializedEditorState }` (or the appropriate Payload rich text type).                                                                                                                                                                                                                                              | ✅ (uses `<RichText data={content} />` in `prose` wrapper)                                                | 2026-03-01 |
| TASK-022 | Update `src/app/[locale]/blog/page.tsx`: No changes to the component structure (it already calls `getAllPosts` and passes to `BlogList`). Verify it compiles with the updated action.                                                                                                                                                                                                                                                                                                                                                                                                                                                     | ✅ (no changes needed; compiles clean)                                                                    | 2026-03-01 |
| TASK-023 | Update `src/app/[locale]/blog/[slug]/page.tsx`: Update `generateStaticParams` to fetch all post slugs from Payload. Update the page to handle Lexical rich text `content` instead of raw MDX string. Pass the Payload rich text content object to `BlogPost` component.                                                                                                                                                                                                                                                                                                                                                                   | ✅ (`generateStaticParams` uses updated `getAllPosts`; `BlogPost` receives Lexical content)               | 2026-03-01 |
| TASK-024 | Remove or deprecate `src/lib/mdx.ts` (the MDX file reader). Remove the `gray-matter` and `next-mdx-remote` dependencies: `pnpm remove gray-matter next-mdx-remote`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | ✅ (`next-mdx-remote` removed; `gray-matter` retained for Phase 4 seed; `lib/index.ts` re-export removed) | 2026-03-01 |
| TASK-025 | Verify blog list page, individual blog post pages, and blog metadata/OG tags all render correctly with Payload data. Test both `en` and `pt` locales.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | ✅ ("No posts yet" confirmed — Payload query works; data populated in Phase 4)                            | 2026-03-01 |
...
| Task     | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Completed                                                                                                   | Date       |
| -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- | ---------- |
| TASK-026 | Create `src/seed/blog.ts`: A script that reads the existing MDX files from `content/blog/en/` and `content/blog/pt/`, parses frontmatter with `gray-matter`, converts MDX body content to Lexical JSON format (using `@payloadcms/richtext-lexical` utilities or manual conversion for simple markdown), and inserts each post into Payload using the Local API `payload.create({ collection: 'posts', locale: 'en', data: { ... } })` then `payload.update({ collection: 'posts', locale: 'pt', id, data: { ... } })` for the Portuguese version. | ✅ (custom MD→Lexical converter; handles h2/h3/h4, ul/ol, code blocks, bold, inline code, links)            | 2026-03-01 |
| TASK-027 | Create `src/seed/index.ts`: Main seed entry point that initializes Payload with `getPayload({ config })`, runs all seed functions (blog first, then portfolio/skills in later phases), and logs results.                                                                                                                                                                                                                                                                                                                                           | ✅                                                                                                          | 2026-03-01 |
| TASK-028 | Add a `seed` script to `package.json`: `"seed": "tsx src/seed/index.ts"` (install `tsx` as devDependency if not present).                                                                                                                                                                                                                                                                                                                                                                                                                          | ✅ (tsx 4.21.0; patched Payload `loadEnv.js` via `pnpm patch` for Next.js 16 compat; `shim.cjs` pre-loader) | 2026-03-01 |
| TASK-029 | Run the seed script, verify all 2 blog posts (hello-world, building-a-layered-frontend-architecture) appear in the Payload admin panel with correct content in both locales.                                                                                                                                                                                                                                                                                                                                                                       | ✅ (both posts seeded with ids 2 & 3; EN + PT locales populated)                                            | 2026-03-01 |
| TASK-030 | After verification, remove `content/blog/` directory (or move to `content/blog.archive/` for safety).                                                                                                                                                                                                                                                                                                                                                                                                                                              | ✅ (deleted; content lives in Payload)                                                                      | 2026-03-01 |
...
| Task     | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Completed                                                                                                 | Date       |
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- | ---------- |
| TASK-031 | Create `src/collections/Projects.ts`: `CollectionConfig` with `slug: 'projects'`, `admin: { useAsTitle: 'title' }`, fields: `title` (text, required, localized), `slug` (text, unique, index), `description` (textarea, required, localized), `thumbnail` (upload, relationTo: 'media'), `techs` (array of text fields, or relationship to 'skills' collection), `demoUrl` (text, optional), `codeUrl` (text, optional), `featured` (checkbox, defaultValue: false), `caseStudy` (group, localized, fields: `enabled` (checkbox), `problem` (textarea, localized), `approach` (textarea, localized), `outcome` (textarea, localized), `learnings` (textarea, localized)). | ✅ (added `sortOrder` number field; `beforeChange` slug hook + `afterChange`/`afterDelete` ISR hooks)     | 2026-03-01 |
| TASK-032 | Register `Projects` collection in `src/payload.config.ts`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | ✅ (already imported + registered in `collections` array)                                                 | 2026-03-01 |
| TASK-033 | Update `src/core/portfolio.ts`: Remove the hardcoded `items` array. Keep the `Portfolio` namespace and `Project` type. Add a `fromPayload(doc)` mapping function. The `Project` type may need to be updated to reference media objects instead of thumbnail filename strings.                                                                                                                                                                                                                                                                                                                                                                                             | ✅ (`PayloadProject` type + `fromPayload` mapping; thumbnail resolves to `{ url, alt }` object)           | 2026-03-01 |
| TASK-034 | Create `src/app/actions/portfolio.ts`: `getAllProjects(locale)` and `getProject(slug, locale)` functions using Payload Local API to fetch from the 'projects' collection.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | ✅ (`getAllProjects` sorts by `sortOrder`; `getProject` by slug; both use `overrideAccess: true`)         | 2026-03-01 |
| TASK-035 | Update `src/ui/portfolio/components/Portfolio.tsx`: Change from importing hardcoded `Portfolio.items` to receiving projects as props or fetching via a server component wrapper. Since this is a `'use client'` component, create a server wrapper `src/app/[locale]/_sections/PortfolioSection.tsx` that fetches data and passes it as props, or convert the home page to pass projects from the server.                                                                                                                                                                                                                                                                 | ✅ (`Portfolio.tsx` converted to `PortfolioSection` async server component; delegates to `PortfolioGrid`) | 2026-03-01 |
| TASK-036 | Update `src/ui/portfolio/components/ProjectCard.tsx`: Update to work with Payload Project type (notably `thumbnail` becomes a media object with `url` instead of a filename string).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | ✅ (renders `thumbnail.url` + `thumbnail.alt` via Next.js `Image`)                                        | 2026-03-01 |
| TASK-037 | Update `src/app/[locale]/portfolio/[slug]/page.tsx`: Replace `Portfolio.items.find()` with `getProject(slug, locale)` from the new action. Update `generateStaticParams` to fetch slugs from Payload. Update image rendering to use Payload media URL.                                                                                                                                                                                                                                                                                                                                                                                                                    | ✅ (`generateStaticParams` uses `getAllProjects`; full detail page with case study, tech stack, links)    | 2026-03-01 |
| TASK-038 | Update `src/app/[locale]/page.tsx` (home page): If the `PortfolioSection` now needs server-fetched data, pass it through or use a server component wrapper.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | ✅ (home page renders `<PortfolioSection />` server component directly; no prop threading needed)         | 2026-03-01 |
...
| Task     | Description                                                                                                                                                                                                                                                                                                                                                                                           | Completed                                                                                                                                                                      | Date       |
| -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------- |
| TASK-039 | Extend `src/seed/portfolio.ts`: Read the 9 portfolio items from the old `Portfolio.items` data (can hardcode in the seed script or import from a snapshot). Upload thumbnail images from `public/images/portfolio/` to the Payload Media collection. Create project documents with localized title/description pulled from the existing `en.json` and `pt.json` message files' `portfolio` namespace. | ✅ (all 9 projects with thumbnails; localized EN+PT; case study for ai-integration-platform; `context: { disableRevalidate: true }` to prevent revalidatePath outside Next.js) | 2026-03-01 |
| TASK-040 | Add portfolio seed to `src/seed/index.ts`.                                                                                                                                                                                                                                                                                                                                                            | ✅                                                                                                                                                                             | 2026-03-01 |
| TASK-041 | Run seed script, verify all 9 projects appear in Payload admin with correct thumbnails, localized text, and case study data.                                                                                                                                                                                                                                                                          | ✅ (ids 2–10; all thumbnails uploaded; EN+PT locales populated)                                                                                                                | 2026-03-01 |
...
| Task     | Description                                                                                                                                                                                                                                                                                                                        | Completed                                                                                                                             | Date       |
| -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- | ---------- |
| TASK-042 | Create `src/collections/Skills.ts`: `CollectionConfig` with `slug: 'skills'`, `admin: { useAsTitle: 'name' }`, fields: `name` (text, required, localized), `slug` (text, unique, index — used for icon lookup), `category` (select: 'frontend'/'backend'/'tools', required), `sortOrder` (number, optional — for manual ordering). | ✅ (`beforeChange` slug hook + `afterChange`/`afterDelete` ISR hooks for home page)                                                   | 2026-03-01 |
| TASK-043 | Register `Skills` collection in `src/payload.config.ts`.                                                                                                                                                                                                                                                                           | ✅                                                                                                                                    | 2026-03-01 |
| TASK-044 | Update `src/core/skills.ts`: Remove hardcoded `items` array, keep namespace and types, add `fromPayload` mapping.                                                                                                                                                                                                                  | ✅ (`PayloadSkill` type + `fromPayload`; `byCategory` updated to accept skills array as first param)                                  | 2026-03-01 |
| TASK-045 | Create `src/app/actions/skills.ts`: `getAllSkills(locale)` and `getSkillsByCategory(category, locale)` using Payload Local API.                                                                                                                                                                                                    | ✅                                                                                                                                    | 2026-03-01 |
| TASK-046 | Update `src/ui/experience/components/Experience.tsx`: Change from importing hardcoded `Skills.byCategory` to receiving data as props from a server component wrapper, or fetch in a parent server component.                                                                                                                       | ✅ (`Experience` accepts `skills` prop; `ExperienceSection.tsx` server wrapper created; home page updated to `<ExperienceSection />`) | 2026-03-01 |
| TASK-047 | Extend `src/seed/skills.ts`: Insert the 12 skills with localized names from message files. Add skills seed to `src/seed/index.ts`.                                                                                                                                                                                                 | ✅ (all 12 skills with EN+PT names; idempotent upsert guard)                                                                          | 2026-03-01 |
| TASK-048 | Run seed, verify skills appear in admin and render correctly on the frontend in both locales.                                                                                                                                                                                                                                      | ✅ (all 12 skills seeded — typescript → docker; `pnpm seed` completes cleanly)                                                        | 2026-03-01 |
...
| Task     | Description                                                                                                                                              | Completed                                                                                                                                                                                                                                                                                            | Date       |
| -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- |
| TASK-049 | Update `Projects` collection: Change `techs` field from array-of-text to `{ name: 'techs', type: 'relationship', relationTo: 'skills', hasMany: true }`. | ✅                                                                                                                                                                                                                                                                                                   | 2026-03-01 |
| TASK-050 | Update portfolio seed script to look up skill IDs and create relationships.                                                                              | ✅ (added `buildSkillsMap` helper that auto-creates missing skills with `showInExperience: false`; migration pass updates existing projects' techs; `TECH_CATEGORY_HINTS` map for categorization; `src/seed/apply-migration.ts` applies raw SQL for schema changes that bypass Drizzle push prompts) | 2026-03-01 |
| TASK-051 | Update `ProjectCard` and portfolio detail page to resolve tech names from the populated relationship.                                                    | ✅ (`depth: 1` added to both portfolio actions; `fromPayload` updated to extract `name` from populated skill objects; `ProjectCard` and detail page unchanged since `project.techs: string[]` API preserved; `showInExperience` field added to Skills collection + `getAllSkills` filter)            | 2026-03-01 |
...
| Task     | Description                                                                                                                                                                                                                                                                                    | Completed                                                                                                                  | Date       |
| -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- | ---------- |
| TASK-052 | Remove `content/blog/` directory after confirming all posts are in Payload.                                                                                                                                                                                                                    | ✅ (deleted in Phase 4 — TASK-030)                                                                                         | 2026-03-01 |
| TASK-053 | Remove `src/lib/mdx.ts` file.                                                                                                                                                                                                                                                                  | ✅ (file deleted; no remaining imports)                                                                                    | 2026-03-01 |
| TASK-054 | Remove `gray-matter` and `next-mdx-remote` from `package.json` dependencies.                                                                                                                                                                                                                   | ✅ (`pnpm remove gray-matter`; `next-mdx-remote` already removed in Phase 3; `seed/blog.ts` updated with inline FM parser) | 2026-03-01 |
| TASK-055 | Remove hardcoded portfolio items from `src/core/portfolio.ts` (keep only type definitions and utility functions).                                                                                                                                                                              | ✅ (already clean — hardcoded items removed in Phase 5)                                                                    | 2026-03-01 |
| TASK-056 | Remove hardcoded skills items from `src/core/skills.ts` (keep only type definitions and utility functions).                                                                                                                                                                                    | ✅ (already clean — hardcoded items removed in Phase 7)                                                                    | 2026-03-01 |
| TASK-057 | Clean up translation files (`en.json`, `pt.json`): Remove portfolio item-specific translation keys (title, description, caseStudy) since those are now in Payload. Keep UI chrome translations (heading, description, labels). Skill name translations can also be removed from message files. | ✅ (removed all 9 per-project portfolio keys and `experience.skill.*` from both locales; UI chrome labels retained)        | 2026-03-01 |
| TASK-058 | Add `revalidateTag` or `revalidatePath` calls in Payload `afterChange`/`afterDelete` hooks to ensure ISR works correctly when content is updated from admin panel.                                                                                                                             | ✅ (`afterChange`/`afterDelete` with `revalidatePath` already present in Posts, Projects, and Skills collections)          | 2026-03-01 |
| TASK-059 | Test full build (`pnpm build`) succeeds with no TypeScript errors.                                                                                                                                                                                                                             | ✅ (`pnpm build` — Compiled successfully; TypeScript clean; 34 pages generated)                                            | 2026-03-01 |
